...
namespace WindowsFormsApplication1
{
    class Lightning
    {
        ...
        public Lightning(int h, int w, Point3D t)
        {
            ...
            for (int i = 0; i < rm.Count; i++)
            {
                ...
                while (l < sublength)
                {
                    ...
                    Transform.RotateY(subl[i], p3D, rm[i]);
                    SubModels[i].Add(p3D);
                }
            }    
        }
        public Lightning(Lightning light)
        {
            ...
            for (int i = 0; i < light.SubModels.Count; i++)
            {
                ...
                for (int j = 0; j < light.SubModels[i].Count; j++)
                    SubModels[i].Add(new Point3D(light.SubModels[i][j].X, light.SubModels[i][j].Y, light.SubModels[i][j].Z));
            }

            for (int j = 0; j < light.Model.Count; j++)
                Model.Add(new Point3D(light.Model[j].X, light.Model[j].Y, light.Model[j].Z));
        }
        public Lightning(IEnumerable<Point3D> model, IEnumerable<Point3D> submodels)
        {
            Model = model.Distinct(new Comparator()).ToList();
            SubModels = new List<List<Point3D>> {new List<Point3D>()};
            SubModels[0] = submodels.Distinct(new Comparator()).ToList();
        }
        public Trace TraceM(Ray ray)
        {
            var t = new Trace {Dist = -1, Per = new Point3D(0,0,0)};
            const double eps = 5;
            foreach (Point3D t4 in Model)
            {
                ...
                if (Math.Abs(t4.Y - y) <= eps)
                {
                    ...
                    {
                        t.Per = new Point3D(t4.X, t4.Y, t4.Z);
                        t.Dist = Math.Sqrt(Math.Pow(t4.X - ray.Beg.X, 2) + Math.Pow(t4.Y - ray.Beg.Y, 2) +
            ...
            foreach (List<Point3D> t4 in SubModels)
            {
                foreach (Point3D t5 in t4)
                {
                    ...
                    if (Math.Abs(t5.Y - y) <= eps)
                    {
                        ...
                        {
                            t.Per = new Point3D(t5.X, t5.Y, t5.Z);
                            t.Dist = Math.Sqrt(Math.Pow(t5.X - ray.Beg.X, 2) + Math.Pow(t5.Y - ray.Beg.Y, 2) +
        ...
        public void RotateLightning(Point3D p, Point3D angle)
        {
            foreach (Point3D t in Model)
            {
                Transform.RotateX(p, t, angle.X);
                Transform.RotateY(p, t, angle.Y);
                Transform.RotateZ(p, t, angle.Z);
            }
            ...
            foreach (List<Point3D> t in SubModels)
            {
                ...
                {
                    Transform.RotateX(p, t1, angle.X);
                    Transform.RotateY(p, t1, angle.Y);
                    Transform.RotateZ(p, t1, angle.Z);
                }
            }
        }
        ...
        public void DrawLightning(PictureBox pictureBox1)
        {
            // pictureBox1.BackColor = Color.Turquoise;
            var l = new Lightning(this);
            //  l.RotateLightning(_center, new Point3D((float)Xa, (float)_ya, (float)Za));
            const int comp = 20;
            ...
            {
                if (Math.Abs(l.Model[i - 1].X - l.Model[i].X) < comp && Math.Abs(l.Model[i - 1].Y - l.Model[i].Y) < comp &&
                    Math.Abs(l.Model[i - 1].Z - l.Model[i].Z) < comp)
                {
                    g.DrawLine(pp, l.Model[i - 1].X, l.Model[i - 1].Y, l.Model[i].X, l.Model[i].Y);
                    Draworeol(new PointF(l.Model[i - 1].X, l.Model[i - 1].Y), new PointF(l.Model[i].X, l.Model[i].Y), g);
                    g.DrawLine(pp, l.Model[i - 1].X - 1, l.Model[i - 1].Y, l.Model[i].X - 1, l.Model[i].Y);
                    g.DrawLine(pp, l.Model[i - 1].X + 1, l.Model[i - 1].Y, l.Model[i].X + 1, l.Model[i].Y);
                }
            }
            ...
            for (int i = 0; i < SubModels.Count; i++)
            {
                ...
                {
                    if (Math.Abs(l.SubModels[i][j - 1].X - l.SubModels[i][j].X) < comp &&
                        Math.Abs(l.SubModels[i][j - 1].Y - l.SubModels[i][j].Y) < comp &&
                        Math.Abs(l.SubModels[i][j - 1].Z - l.SubModels[i][j].Z) < comp)
                    {
                        g.DrawLine(pp, l.SubModels[i][j - 1].X, l.SubModels[i][j - 1].Y, l.SubModels[i][j].X,
                                   l.SubModels[i][j].Y);
                        Draworeol(new PointF(l.SubModels[i][j - 1].X, l.SubModels[i][j - 1].Y),
                                  new PointF(l.SubModels[i][j].X, l.SubModels[i][j].Y), g);
                    }
                }
            }
        ...
        public void DrawArea(Graphics g, Point3D p)
        {
