...
namespace WindowsFormsApp1
{
    ...
    class House
    {
        public House(int w, int h, Point3D c, int angle, int count)
        {
            ...
            WindowsCreate(c);
            TurnHouse(_center, new Point3D(0, angle, 0));
        }

        private void WindowsCreate(Point3D c)
        {
            W = new Window[4][];
            var p = new Point3D(S[1].P[0].X + 10,
                                S[1].P[0].Y - 10,
                                S[1].P[0].Z);
            for (int i = 0; i < 4; i++)
            {
            ...
            for (int i = 0; i < _kol; i++)
            {
                ...
                W[1][k] = new Window(p);
                W[2][k] = new Window(p);
                W[2][k].TurnW(new Point3D(0, 90, 0), c);
                W[3][k] = new Window(p);
        ...
        public Trace TraceM(Ray ray)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                // определяется параметр t  при котором луч пересекается плоскость
                ...
                // проверка на пренадлежность плоскости 
                if (per.X >= Math.Min(S[i].P[0].X, S[i].P[2].X) &&
                    per.X <= Math.Max(S[i].P[0].X, S[i].P[2].X) &&
                    per.Y >= Math.Min(S[i].P[0].Y, S[i].P[2].Y) &&
                    //per.Y <= Math.Max(S[i].P[0].Y, S[i].P[2].Y) &&
                    per.Z >= Math.Min(S[i].P[0].Z, S[i].P[2].Z) &&
                    per.Z <= Math.Max(S[i].P[0].Z, S[i].P[2].Z))
                {
                    // расстояние от точки до начала луча
                    double dist = Math.Sqrt(Math.Pow(per.X - ray.Beg.X, 2) +
                                            Math.Pow(per.Y - ray.Beg.Y, 2) +
                                            Math.Pow(per.Z - ray.Beg.Z, 2));
                    // сравнение с минимальным расстоянием
                    if (dist < min.Dist || min.Dist == -1)
                    {
                        // если расстяние меньше, то создаю новую структуру Trace
        ...
        private void TurnHouse(Point3D p, Point3D angle)
        {
            for (int i = 0; i < 8; i++)
                for (int j = 0; j < 4; j++)
                {
                    ...
                    Transform.TurnX(p, S[i].P[j], angle.X);
                    Transform.TurnZ(p, S[i].P[j], angle.Z);
                }
            for (int i = 0; i < 4; i++)
            ...
            Transform.TurnY(p, La[0], angle.Y);
            Transform.TurnX(p, La[0], angle.X);
            Transform.TurnZ(p, La[0], angle.Z);
            Transform.TurnY(p, La[1], angle.Y);
