...
namespace WindowsFormsApp1
{
    struct Trace
    {
        public Point3D Per; // точка пересечения 
        public double Dist; // расстояние до объекта
        public int I; // с чем пересекается
    }

    struct Ray
    {
        public Point3D Vec; // куда верктор направлен (точка на луче)
        public Point3D Beg; // начало луча
    ...
    class RayTracer 
    {
        ...
        {
            // по каждой точке окна при помощи алгоритма Брезенхема
            // чтобы получить отраженный луч нам надо повернуь вектор нормали плоскости
            // окна на градус угла между (лучом от камеры до точки) и (поскости окна)
            // вокруг вектора, равного результату векторного произведения исходного луча
            // и вектора нормали окна
            int count_ray = 0;
            ...
            for (r = 0.0; r <= 1; r = r + step)
            {
                ...
                for (r2 = 0.0; r2 <= 1; r2 = r2 + step2)
                {
                    ...
                    // направляющий вектор луча от камеры до точки
                    var poi = new Point3D(x3 - po.X, y3 - po.Y, z3 - po.Z);
                    // угол между лучом и плоскостью окна
                    ...
                    // Ось вращения вокруг которой идет поворот для нахождения
                    // отраженного луча
                    ...
                    // поворот вектора нормали к плоскости на грудус А вокруг оси вращения 
                    // тем самымм получаем отраженный луч
                    ...
                    // ищем точку пересечения луча с молнией
                    Trace t2 = lightning.TraceM(ray);

                    // если пересекает, то заносим в массив "отражение"
                    if (t2.Dist != -1)
                    {
