...
namespace WindowsFormsApp1
{
    class Lightning
    {
        ...
        {
            var subl = new List<Point3D>(); // поб ветвь
            var r = new Random();
            var rm = new List<int>();
            bool f = true;
            float x = r.Next(0, w);
        ...
        public Trace TraceM(Ray ray)
        {
            ...
            {
                // параметр t  при котором Х сегмента совпадает с Х луча
                double t1 = (t4.X - ray.Beg.X) / ray.Vec.X;
                // У предполагаемой точки пересечения
                double y = ray.Beg.Y + ray.Vec.Y * t1;
                // У проверяется с найденным с погрешностью
                if (Math.Abs(t4.Y - y) <= eps)
                {
                    // Z предполагаемой точки пересечения
                    double z = ray.Beg.Z + ray.Vec.Z * t1;
                    // проверяем Z 
                    if (Math.Abs(t4.Z - z) <= eps)
                    {
                        // если все совпадает, то возвращается точка пересечения
            ...
            // все тоже саоме только с ветвями
